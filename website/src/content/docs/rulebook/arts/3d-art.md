---
title: 3D Art
description: Rules, procedures, and rubric for MIST 3D Art competition
sidebar:
  order: 2
---

Think you have what it takes to be the next great sculptor? Is clay your ultimate inspiration? Show us your skills in the 3D art competition.

<div class="competition-badges">
  <span class="badge badge-format">In-Person</span>
  <span class="badge badge-type">Individual</span>
</div>

## On-Site Submission Requirements
- Artwork must be prepared and ready when you arrive at the tournament
- Competitors must drop off their completed product to the allocated room upon registration
- Written artist statement (3 hard copies, 500 words maximum)
- Pictures of Artwork (JPG, TIFF, or PNG format) - as many as needed from varying angles

## Registration Guidelines

1. No more than **four (4) students** from each school can register for this competition.
2. This competition is **NOT** a team effort. Each competitor must turn in a separate piece of work.
3. MIST ID and order of competition will be pre-assigned on a random selection basis.

## Additional Rules for 3D Art

The 3D Art competition follows the same rules as 2D Art with these additional requirements:

1. All submissions must be **three-dimensional**.

2. Artwork should be placed on a **board for easy carrying**. The board can be made of any material sturdy enough to carry the artwork.

3. **3D art should be sturdy.** Judges are looking for artwork that is well put together. Your work may be moved around on several occasions and should be robust to handle these multiple moves.

4. **3D printing will NOT be permitted** for any aspect of the submission.
   - Usage of 3D printing is grounds for elimination from the competition.

5. Students **MUST** include pictures of their work at multiple points of the planning and creation process with their art piece along with their written component.

6. There is no limitation to the size of the submission as long as it can be safely carried to the venue.

## Checklist
- [ ] 3D Art submission
- [ ] Pictures showing art process at multiple stages
- [ ] Written statement based on interview questions (Section D of ballot)
- [ ] Both submission and written statement labeled with MIST ID and competition name

:::tip
We recommend making two copies of your written statement: one to attach to your 3D submission, and the required separate statement collected at on-site registration.
:::

## Interview Questions

Judges will ask:
1. How did you aim to convey this year's theme in your work? In what ways does your artwork represent your individual point of view, imagination, creativity, and individuality?
2. Explain your purpose in developing this artwork and describe how you achieved that goal. Describe any influences from art history, artists, or art periods or styles that contributed to the development of this artwork.
3. What elements or principles of art did you use, and how did you select and organize the visual aspects of this work?

## Scoring Rubric

| Section | Category | Points |
|---------|----------|--------|
| A | Technical | /5 |
| B | Purpose | /15 |
| B | Skills and Technique | /15 |
| C | Aesthetic and Design | /20 |
| C | Creativity | /20 |
| D | Application of Theme | /25 |
| **Total** | | **/100** |
